...
# ===== Tools Stage =====
FROM public.ecr.aws/amazonlinux/amazonlinux:2023.12.20260727.0@sha256:601289f29d1b67fc542a451beda8ac6ca542d40289c99f71faa3ba37fe0d133a AS tools
...
SHELL ["/bin/bash", "-o", "pipefail", "-c"]
RUN dnf install -y tar-1.34 gzip-1.12 && \
    dnf clean all && \
    rm -rf /var/cache/dnf && \
    curl -fsSL --retry 5 --retry-connrefused --retry-delay 5 --connect-timeout 5 \
...
RUN dnf install -y shadow-utils-4.9 && \
    groupadd -r -g 1000 mailbaku && \
    useradd -r -u 1000 -g mailbaku -d /app -s /sbin/nologin mailbaku && \
...
COPY --from=build --chown=mailbaku:mailbaku /app/target/quarkus-app/ /app/
